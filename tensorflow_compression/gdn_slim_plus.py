...
class GDN_slim_plus(base.Layer):
  ...
  def __init__(self,
               ...
               scale_gamma_parameterizer=_default_scale_gamma,
               bias_gamma_parameterizer=_default_bias_gamma,
               scale_beta_parameterizer=_default_scale_beta,
               bias_beta_parameterizer=_default_bias_beta,
               activity_regularizer=None,
               trainable=True,
               name=None,
               **kwargs):
    super(GDN_slim_plus, self).__init__(trainable=trainable, name=name,
    ...
    self._scale_gamma_parameterizer = scale_gamma_parameterizer
    self._bias_gamma_parameterizer = bias_gamma_parameterizer
    self._scale_beta_parameterizer = scale_beta_parameterizer
    self._bias_beta_parameterizer = bias_beta_parameterizer 
  ...
  def build(self, input_shape):
    ...
    self.beta_scale = self._scale_beta_parameterizer(
        name="beta_scale", shape=[self._num_deform], dtype=self.dtype,
        getter=self.add_variable, initializer=init_ops.Ones())

    self.beta_bias = self._bias_beta_parameterizer(
        name="beta_bias", shape=[self._num_deform], dtype=self.dtype,
        getter=self.add_variable, initializer=init_ops.Zeros())

    self.gamma_scale = self._scale_gamma_parameterizer(
        name="gamma_scale", shape=[self._num_deform], dtype=self.dtype,
        getter=self.add_variable, initializer=init_ops.Ones())

    self.gamma_bias = self._bias_gamma_parameterizer(
        name="gamma_bias", shape=[self._num_deform], dtype=self.dtype,
        getter=self.add_variable, initializer=init_ops.Zeros())

    self.built = True


  def call(self, inputs, idx, mask):
    inputs = inputs[:,:,:,:mask]
    ...
    if ndim == 2:
      ...
      norm_pool = nn.convolution(math_ops.square(inputs), self.gamma_scale[idx]*gamma[:,:,:mask,:mask]+self.gamma_bias[idx], "VALID")
      norm_pool = nn.bias_add(norm_pool, self.beta_scale[idx]*self.beta[:mask]+self.beta_bias[idx])
    else:  # generic implementation
      # This puts channels in the last dimension regardless of input.
      norm_pool = math_ops.tensordot(
          math_ops.square(inputs), self.gamma_scale[idx]*self.gamma[:,:,:mask,:mask]+self.gamma_bias[idx], [[self._channel_axis()], [0]])
