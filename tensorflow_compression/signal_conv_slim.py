...
class _SignalConv(base.Layer):
  ...
  def call(self, inputs, mask_in, mask_out):
    ...
    if (corr and
      ...
      outputs = nn.depthwise_conv2d_native(
          outputs, kernel[:,:,:mask_in,:mask_out], strides=self._pad_strides(self.strides_down),
      ...
      outputs = nn.convolution(
          outputs, kernel[:,:,:mask_in,:mask_out], strides=self.strides_down, padding="VALID",
      ...
      if self._rank == 1 and not self.channel_separable:
        ...
        temp = nn.depthwise_conv2d_native_backprop_input(
            temp_shape, kernel[:,:,:mask_out, :mask_in], outputs,
        ...
        temp = nn.conv2d_transpose(outputs, kernel[:,:,:mask_out,:mask_in], temp_shape,strides=self._pad_strides(self.strides_up), padding="VALID",data_format=data_format)
    ...
    if self.bias is not None:
      if self.data_format == "channels_first":
        ...
        if self._rank == 1:
          ...
          outputs = nn.bias_add(outputs, self.bias[:mask_out], data_format="NCHW")
        elif self._rank >= 3:
        ...
        outputs = nn.bias_add(outputs, self.bias[:mask_out])
...
SignalConv2D_slim = _conv_class_factory("SignalConv2D", 2)
